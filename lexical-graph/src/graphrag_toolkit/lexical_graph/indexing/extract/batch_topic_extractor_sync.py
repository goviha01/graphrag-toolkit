...
class BatchTopicExtractorSync(BaseExtractor):
    ...
    def _process_single_batch(self, batch_index:int, node_batch:List[TextNode], s3_client, bedrock_client):
        try:
            ...
            input_filename = f'topic-extraction-{timestamp}-batch-{batch_suffix}.jsonl'
            ...
            batch_end = time.time()
            raise BatchJobError(f'[ProTopicosition batch] Error processing batch {batch_index} ({int(batch_end-batch_start)} seconds): {str(e)}') from e 
    ...
    def _process_nodes(self, 
        ...
        **kwargs: Any
        ...
        if len(nodes) < BEDROCK_MIN_BATCH_SIZE:
            ...
            logger.info(f'[Topic batch] Not enough records to run batch extraction. List of nodes contains fewer records ({len(nodes)}) than the minimum required by Bedrock ({BEDROCK_MIN_BATCH_SIZE}), so running TopicExtractor instead.')
