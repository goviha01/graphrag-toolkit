...
from graphrag_toolkit.lexical_graph.metadata import VALID_FROM, VALID_TO, VERSIONING_FIELDS, EXTRACT_TIMESTAMP
...
class VersionManager(NodeHandler):
    ...
    def _get_other_source_versions(self, node:BaseNode) -> List[Dict[str, Any]]:
        ...
        for node in node_iterable:
            ...
            if [key for key in [INDEX_KEY] if key in node.metadata]:
                ...
                if index_name == 'source':
                    ...
                    other_source_versions = self._get_other_source_versions(node)
                    ...
                    for source_id, valid_to in previous_versions.items():
                        for index in self.vector_store.all_indexes():
                            ...
                            for source_id, node_ids in node_id_map.items():
                                ...
                                self._set_source_node_version_info(source_id, valid_to)
